                ...
                <div class="contact-form" style="background-color: white;padding: 15px;border-radius: 10px">
                    ...
                    <div class="row">
                        ...
                        <?php if (count($kode_etik_question) > 0): ?>
                            <?php $no = 0; ?>
                            <?php $__currentLoopData = $kode_etik_question; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $kode_etik_question): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <div id="div_kodeetik_question_<?= $no ?>" style="display: none;width: 100%;padding: 15px;" class="col-xs-12">
                                <label class="label-input1002" style="color: purple;margin-top: 0px;font-size: 14px"><span id="kodeetik_question_<?= $no ?>"><?php echo e($no+1); ?>. <?php echo e($kode_etik_question->question); ?></span></label>
                                ...
                                <?php for ($i=0; $i < count($kodeetik_answer); $i++) { ?>
                                    <div class="validate-input" style="position: relative; width: 100%">
                                        <label class="radio_box" style="margin-top: 5px;font-size: 12px"><?php echo e($kodeetik_answer[$i]); ?>

                                            <input type="radio" id="kodeetik_answer_<?php echo e($no); ?>" name="kodeetik_answer_<?php echo e($no); ?>" value="<?php echo e($kodeetik_answer[$i]); ?>">
                                            <span class="checkmark_box"></span>
                                        </label>
                                    </div>
                                <?php } ?>
                                <input type="hidden" name="kodeetik_right_answer_<?php echo e($no); ?>" id="kodeetik_right_answer_<?php echo e($no); ?>" value="<?php echo e($kode_etik_question->right_answer); ?>">
                                <br>
                            </div>
                            ...
                            <button class="main-button" type="button" id="btn_kodeetik_submit_<?= $no ?>" onclick="submitKodeEtikQuestion('<?php echo e($no); ?>')" style="display: inline-block;float: right;display: none;margin: 20px">
                                <span>
                                    Submit
                                    <i class="fa fa-arrow-right"></i>
                                </span>
                            </button>
                            <button class="main-button" type="button" id="btn_kodeetik_back_<?= $no ?>" onclick="backKodeEtikQuestion('<?php echo e($no); ?>')" style="display: inline-block;float: right;display: none;background-color: red;margin: 20px">
                                <span>
                                    <i class="fa fa-arrow-left"></i>
                                    Back
                                </span>
                            </button>
                            <button class="main-button" type="button" id="btn_kodeetik_next_<?= $no ?>" onclick="nextKodeEtikQuestion('<?php echo e($no); ?>')" style="display: inline-block;float: right;display: none;margin: 20px">
                                <span>
                                    Next
                                    <i class="fa fa-arrow-right"></i>
                                </span>
                    ...
                    <center><span style="font-size: 18px;text-align: center;color: black;">
                        <?php date_default_timezone_set('Asia/Jakarta'); ?>
                        Terimakasih <span style="color: blue;"><?php echo e($empsync[0]->name); ?></span><span style="color: black;"> telah melakukan Training Kode Etik Kepatuhan</span><br> pada<br>
                        <?php if (ISSET($cek_kode_etik) && $cek_kode_etik != null){ ?>
                            <span id="tanggal_kode_etik" style="font-weight: bold;color: red;"><?php echo e($cek_kode_etik->created_at); ?></span> 
                        <?php }else{ ?>
...
<script type="text/javascript">
    ...
    jQuery(document).ready(function() {
        for(var i = 0; i < parseInt('<?php echo e($kodeetik_question_total); ?>');i++){
        ...
        if (parseInt('<?php echo e(count($cek_kode_etik)); ?>') > 0) {
            $('#title_sambutan').hide();
    ...
    function nextKodeEtikQuestion(no) {
        ...
        if ((parseInt(no)+1) == '<?php echo e($kodeetik_question_total); ?>') {
            ...
            $.get('<?php echo e(url("kodeetik/input")); ?>', data, function(result, status, xhr){
