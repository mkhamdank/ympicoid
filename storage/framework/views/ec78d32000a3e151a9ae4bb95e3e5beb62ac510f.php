        ...
        <div class="row">
            ...
            <div class="col-lg-12" style="margin-top:20px;">
                <div class="center-heading">
                    ...
                    <h4 class="section-title" style="background-color: white;font-size: 12px;border-bottom-left-radius: 10px;border-bottom-right-radius: 10px;margin-bottom: 10px">PERIODE <?php echo e($periode); ?></h4>
                ...
                <div class="contact-form" style="background-color: white;padding: 15px;border-radius: 10px">
                    ...
                    <input type="hidden" name="periode" id="periode" value="<?php echo e($periode); ?>">
                    ...
                    <div class="row">
                        ...
                        <?php if (count($pkb_question) > 0): ?>
                            <?php $no = 0; ?>
                            <?php $__currentLoopData = $pkb_question; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $pkb_question): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <div id="div_pkb_question_<?= $no ?>" style="display: none;width: 100%;padding: 15px;" class="col-xs-12">
                                <label class="label-input1002" style="color: purple;margin-top: 0px;font-size: 14px"><span id="pkb_question_<?= $no ?>"><?php echo e($no+1); ?>. <?php echo e($pkb_question->question); ?></span></label>
                                ...
                                <?php for ($i=0; $i < count($pkb_answer); $i++) { ?>
                                    <div class="validate-input" style="position: relative; width: 100%">
                                        <label class="radio_box" style="margin-top: 5px;font-size: 12px"><?php echo e($pkb_answer[$i]); ?>

                                            <input type="radio" id="pkb_answer_<?php echo e($no); ?>" name="pkb_answer_<?php echo e($no); ?>" value="<?php echo e($pkb_answer[$i]); ?>">
                                            <span class="checkmark_box"></span>
                                        </label>
                                    </div>
                                <?php } ?>
                                <input type="hidden" name="pkb_right_answer_<?php echo e($no); ?>" id="pkb_right_answer_<?php echo e($no); ?>" value="<?php echo e($pkb_question->right_answer); ?>">
                                <br>
                            </div>
                            ...
                            <button class="main-button" type="button" id="btn_pkb_submit_<?= $no ?>" onclick="submitPkbQuestion('<?php echo e($no); ?>')" style="display: inline-block;float: right;display: none;margin: 20px">
                                <span>
                                    Submit
                                    <i class="fa fa-arrow-right"></i>
                                </span>
                            </button>
                            <button class="main-button" type="button" id="btn_pkb_back_<?= $no ?>" onclick="backPkbQuestion('<?php echo e($no); ?>')" style="display: inline-block;float: right;display: none;background-color: red;margin: 20px">
                                <span>
                                    <i class="fa fa-arrow-left"></i>
                                    Back
                                </span>
                            </button>
                            <button class="main-button" type="button" id="btn_pkb_next_<?= $no ?>" onclick="nextPkbQuestion('<?php echo e($no); ?>')" style="display: inline-block;float: right;display: none;margin: 20px">
                                <span>
                                    Next
                                    <i class="fa fa-arrow-right"></i>
                                </span>
                    ...
                    <div class="row" id="surat_pernyataan" style="padding: 10px">
                        ...
                        <div id="div_detail" class="col-xs-12" style="width: 100%">
                            ...
                            <br>
                            <br>
                            Pasuruan, <?php echo e(date('d F Y')); ?>
                    ...
                    <div id="div_detail_sudah" style="margin-top: 20px">
                        <center><span style="font-size: 18px;text-align: center;color: green">
                            ...
                            <?php if (ISSET($cek_pkb) && $cek_pkb != null){ ?>
                                <span id="tanggal_pkb" style="font-weight: bold;"><?php echo e($cek_pkb->created_at); ?></span>
                            <?php }else{ ?>
...
<script type="text/javascript">
    ...
    jQuery(document).ready(function() {
        for(var i = 0; i < parseInt('<?php echo e($pkb_question_total); ?>');i++){
        ...
        $('#nama').html('<?php echo e($empsync[0]->name); ?>');
        $('#nama_bawah').html('<?php echo e($empsync[0]->name); ?>');
        $('#nik').html('<?php echo e($empsync[0]->employee_id); ?>');
        $('#department_pkb').html('<?php echo e($empsync[0]->department); ?>');
        $('#grade').html('<?php echo e($empsync[0]->grade_code); ?>');
        $('#jabatan').html('<?php echo e($empsync[0]->position); ?>');

        if (parseInt('<?php echo e(count($cek_pkb)); ?>') > 0) {
    ...
    function backPkbQuestion(no) {
      ...
      function nextPkbQuestion(no) {
        ...
        if ((parseInt(no)+1) == '<?php echo e($pkb_question_total); ?>') {
            $('#div_pertanyaan').hide();
            $('#btn_pkb_submit_all').show();
            $('#surat_pernyataan').show();
            $('#title_pertanyaan').hide()
            alert('Anda berhasil menyelesaikan pertanyaan PKB Periode '+'<?php echo e($periode); ?>'+'. Silahkan Sign SURAT PERNYATAAN berikut.');
        }
      }
    function savePkb() {
        $("#loading").show();
        ...
        $.get('<?php echo e(url("pkb/input")); ?>', data, function(result, status, xhr){
